       ...
       // System.out.println(data.toString());
        for(int i =0; i < data.length(); i++){
            try {
                ...
                if(!TextUtils.isEmpty(obj.getString("dial_code")) && !obj.getString("dial_code").equals("null")) {
        ...
        Collections.sort(countries, new Comparator<Country>() {
            ...
            public int compare(Country country1, Country country2) {
                String c1 =  country1.getDial_code().replace("+","").replaceAll("\\s","");
                String c2 =  country2.getDial_code().replace("+","").replaceAll("\\s","");
                int ii1 = Integer.valueOf(c1);
                int ii2 = Integer.valueOf(c2);
